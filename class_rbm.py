...
class class_RBM:
    def __init__(self, N, M, n_cl, eta=1e-1, batch_size=100, W_init='std'):
        self.N = N                # number of visible units
        self.M = M                # number of hidden units
        self.n_cl = n_cl          # number of classes C
        self.W_init = W_init
    ...
    def sample_v_given_h(self, h):
        ...
        prob_l = cp.true_divide(cp.exp(x), cp.expand_dims(cp.sum(np.exp(x), axis=1),axis=1))
    ...
    def sample_class_given_v(self, input_data):
        ...
        for y in range(num_classes):
            ...
            for j in range(num_hidden):
                prod += cp.log(1 + np.exp(precomputed_factor[:,j] + class_weights[y, j]))
